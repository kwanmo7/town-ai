CREATE TABLE `report` (
    ...
    `report_type` VARCHAR(10) NOT NULL COMMENT 'SUMMARY / ALL / AREA / COMPARE',
    `model` VARCHAR(50) NOT NULL COMMENT 'AI 모델',
    `prompt_version` VARCHAR(30) NOT NULL COMMENT 'AI 프롬프트 버전',
    `storage_path` VARCHAR(255) NULL COMMENT 'Report ID 선점 Transaction 안에서만 임시 NULL 허용',
...
CREATE TABLE `line_webhook_event` (
    ...
    `status` VARCHAR(20) NOT NULL COMMENT 'RECEIVED / PROCESSING / COMPLETED / FAILED',
    ...
    `processing_started_at` TIMESTAMP NULL COMMENT '현재 처리 시도 시작 시각 UTC',
...
CREATE TABLE `line_visit_draft` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'id',
    `source_webhook_event_id` VARCHAR(64) NOT NULL COMMENT 'Draft를 생성한 LINE webhookEventId',
    ...
    `status` VARCHAR(30) NOT NULL COMMENT 'NEEDS_INPUT / AWAITING_CONFIRMATION / CONFIRMED / CANCELLED / EXPIRED',
...
ALTER TABLE `line_webhook_event` ADD CONSTRAINT `PK_LINE_WEBHOOK_EVENT` PRIMARY KEY (
    `webhook_event_id`
);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `PK_LINE_VISIT_DRAFT` PRIMARY KEY (
    `id`
);
...
ALTER TABLE `line_visit_draft` ADD CONSTRAINT `UK_LINE_VISIT_DRAFT_SOURCE_EVENT` UNIQUE (
    `source_webhook_event_id`
);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `UK_LINE_VISIT_DRAFT_CONFIRMED_VISIT` UNIQUE (
    `confirmed_visit_id`
);

ALTER TABLE `visit` ADD CONSTRAINT `CHK_VISIT_ATMOSPHERE_SCORE`
    CHECK (`atmosphere_score` BETWEEN 0 AND 10);

ALTER TABLE `visit` ADD CONSTRAINT `CHK_VISIT_INFRA_SCORE`
    CHECK (`infra_score` BETWEEN 0 AND 10);

ALTER TABLE `visit` ADD CONSTRAINT `CHK_VISIT_CLEAN_SCORE`
    CHECK (`clean_score` BETWEEN 0 AND 10);

ALTER TABLE `visit` ADD CONSTRAINT `CHK_VISIT_SIZE_SCORE`
    CHECK (`size_score` BETWEEN 0 AND 10);

ALTER TABLE `visit` ADD CONSTRAINT `CHK_VISIT_ACCESS_SCORE`
    CHECK (`access_score` BETWEEN 0 AND 10);

ALTER TABLE `line_webhook_event` ADD CONSTRAINT `CHK_LINE_WEBHOOK_EVENT_TYPE`
    CHECK (`event_type` IN ('TEXT_MESSAGE', 'POSTBACK'));

ALTER TABLE `line_webhook_event` ADD CONSTRAINT `CHK_LINE_WEBHOOK_EVENT_PAYLOAD`
    CHECK (
        (`event_type` = 'TEXT_MESSAGE' AND `message_text` IS NOT NULL AND `postback_data` IS NULL)
        OR
        (`event_type` = 'POSTBACK' AND `message_text` IS NULL AND `postback_data` IS NOT NULL)
    );

ALTER TABLE `line_webhook_event` ADD CONSTRAINT `CHK_LINE_WEBHOOK_EVENT_STATUS`
    CHECK (`status` IN ('RECEIVED', 'PROCESSING', 'COMPLETED', 'FAILED'));

ALTER TABLE `line_webhook_event` ADD CONSTRAINT `CHK_LINE_WEBHOOK_EVENT_ATTEMPT_COUNT`
    CHECK (`attempt_count` >= 0);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_VISIT_DRAFT_STATUS`
    CHECK (`status` IN ('NEEDS_INPUT', 'AWAITING_CONFIRMATION', 'CONFIRMED', 'CANCELLED', 'EXPIRED'));

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_VISIT_DRAFT_CONFIRMED_VISIT`
    CHECK (
        (`status` = 'CONFIRMED' AND `confirmed_visit_id` IS NOT NULL)
        OR
        (`status` <> 'CONFIRMED' AND `confirmed_visit_id` IS NULL)
    );

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_DRAFT_ATMOSPHERE_SCORE`
    CHECK (`atmosphere_score` IS NULL OR `atmosphere_score` BETWEEN 0 AND 10);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_DRAFT_INFRA_SCORE`
    CHECK (`infra_score` IS NULL OR `infra_score` BETWEEN 0 AND 10);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_DRAFT_CLEAN_SCORE`
    CHECK (`clean_score` IS NULL OR `clean_score` BETWEEN 0 AND 10);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_DRAFT_SIZE_SCORE`
    CHECK (`size_score` IS NULL OR `size_score` BETWEEN 0 AND 10);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `CHK_LINE_DRAFT_ACCESS_SCORE`
    CHECK (`access_score` IS NULL OR `access_score` BETWEEN 0 AND 10);
...
ALTER TABLE `line_visit_draft` ADD CONSTRAINT `FK_LINE_DRAFT_SOURCE_EVENT` FOREIGN KEY (
    `source_webhook_event_id`
) REFERENCES `line_webhook_event` (
    `webhook_event_id`
);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `FK_LINE_DRAFT_AREA` FOREIGN KEY (
    `area_id`
) REFERENCES `area` (
    `id`
);

ALTER TABLE `line_visit_draft` ADD CONSTRAINT `FK_LINE_DRAFT_CONFIRMED_VISIT` FOREIGN KEY (
    `confirmed_visit_id`
) REFERENCES `visit` (
    `id`
);
