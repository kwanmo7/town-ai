...
- `currentDate`: 상대적인 날짜 표현을 해석하기 위한 Backend 기준일
- `text`: 사용자가 입력한 자연어
...
```json
{
  ...
  "verificationChecklist": [
    {
      "category": "통근",
      "content": "평일 출근 시간대의 실제 이동 시간과 환승 횟수를 두 지역에서 동일한 조건으로 확인합니다."
    },
    {
      "category": "주거비",
...
- 객관 자료가 입력되지 않은 상태에서 임대료, 통근 시간, 치안 또는 재해 위험을 사실로 단정하지 않는다.
