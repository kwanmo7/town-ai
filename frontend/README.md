...
- 공개 Component, Hook, API Type에는 역할과 사용자 흐름을 설명하는 한국어 TSDoc을 작성한다.
- 인증 경계, AI 결과 검토, 상태 전환, API 오류 변환처럼 코드만으로 이유가 드러나지 않는
  부분에는 구현 이유를 인접 주석으로 남긴다.
- JSX 구조, 단순 상태 Setter, 이름만으로 충분한 변환 로직을 그대로 번역하는 주석은 작성하지
  않는다.
- Test의 `describe`·`it` 이름은 동작과 기대 결과가 드러나게 작성하고, Mock 제약처럼 이름으로
  설명할 수 없는 경우에만 주석을 추가한다.
